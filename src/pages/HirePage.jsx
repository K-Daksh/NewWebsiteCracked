...
export const HirePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            setFormData({ companyName: '', workEmail: '', requirements: '' });
        } else {
  ...
  const handleWhatsAppChat = () => {
    if (settings?.phone) {
        let cleanNumber = settings.phone.replace(/[^0-9]/g, '');
        
        if (cleanNumber.length === 10) {
            cleanNumber = '91' + cleanNumber;
        }

        // Distinctly open in new tab with security features
        window.open(`https://wa.me/${cleanNumber}`, '_blank', 'noopener,noreferrer');
    } else {
        alert("Phone number not configured in Admin Dashboard.");
    }
  };
  ...
  return (
    <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                    <div className="space-y-1.5 md:space-y-2">
                      ...
                      <input 
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-xl p-3 md:p-4 text-white focus:outline-none focus:border-purple-500 focus:bg-white/10 transition-all placeholder:text-white/20 text-xs md:text-sm" 
                ...
                <div className="space-y-1.5 md:space-y-2">
                  ...
                  <textarea 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl p-3 md:p-4 text-white focus:outline-none focus:border-purple-500 focus:bg-white/10 transition-all placeholder:text-white/20 text-xs md:text-sm resize-none" 
                      placeholder="We need a React Native developer with 3+ years experience..." 
                  />
                </div>
                ...
                <LiquidButton 
                  ...
                  rounded="rounded-xl" 
                  className={`w-full text-white font-semibold py-3 md:py-5 text-sm md:text-lg ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isSubmitting ? 'Sending Request...' : 'Send Hiring Request'}
                </LiquidButton>
                ...
                <div className="flex items-center justify-center gap-2 md:gap-3 pt-4 md:pt-6 border-t border-white/10 mt-6 md:mt-8">
                  ...
                  <button type="button" onClick={handleWhatsAppChat} className="text-green-400 font-medium hover:text-green-300 transition-colors flex items-center gap-2 text-xs md:text-sm">
      ...
      {/* Toast Notification - Using Portal for true fixed positioning */}
      {showToast && ReactDOM.createPortal(
          <motion.div
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50 }}
              className="fixed bottom-6 right-6 z-[9999] bg-[#0a0a0a] border border-green-500/20 text-white px-6 py-4 rounded-xl shadow-[0_0_30px_rgba(34,197,94,0.1)] flex items-center gap-4 max-w-md"
          >
              ...
              <div>
                  ...
                  <p className="text-white/50 text-xs mt-0.5">Our team will reach out to you as soon as possible.</p>
              </div>
          </motion.div>,
          document.body
      )}
